 ...
 @item{@bold{Space Invaders!}
  ...
  @item{@bold{Finger exercises: Designing classes}
	...
	means that trees with two or three values cannot be represented.
        ...
        Implement the methods @tt{size} (how many numbers are there in
	the tree), @tt{sum} (sums all the numbers in the tree),
	@tt{prod} (computes the product of all the numbers in the
	tree), @tt{contains?} (is a given number in the tree?),
	@tt{map} (apply a function to every number in the tree), and
	@tt{max} (what's the largest number in the tree?).}  ]
