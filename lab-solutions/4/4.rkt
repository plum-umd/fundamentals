...
(define-class world%
  ...
  (define/public (on-key k)
    (cond
      [(key=? k "g") (make-package this -50i)]
      [(key=? k "s") (make-package this 0)])))
...
(define (hash-map-values h f)
  (make-immutable-hash (dict-map h (λ (k v) (list k (f v))))))
...
(define-class universe%
  ...
  (define/public (on-new iw)
    (just-universe
      (this . with-ships (dict-set (field ships) iw (random-ship)))))
  ...
  (define/public (dead-worlds)
    (apply append
           (dict-map (field ships)
                     (λ (iw s)
                       (if (ormap (λ (r) (overlap? s r)) (field rocks))
                         (list iw)
                         empty)))))
