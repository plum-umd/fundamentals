...
This is the definition of a class with a {\tt fact} method:
...
We need to rewrite this method, using the \verb|Natural| interface,
...
;; - sub1 : -> Natural
;; produce a Natural one less than this one 
;; (or zero if this one is zero)
;; - times : Natural -> Natural
;; multiply this Natural by the given one
\end{verbatim}

Write a new definition of the \verb|fact%| class using \verb|Natural|,
and \emph{not} using the built-in numbers of the class system or
DrRacket.  You can use the constant \verb|one|, which is a \verb|Natural|
representing $1$.
...
supports the methods \verb|zero?|, \verb|sub1|, and \verb|times|.  Define the
constant \verb|one|.
...
;; run the first action if this is true, otherwise run the second action
...
reimplement the \verb|fact%| class to use them.
...
(define-class fact%
  (define (fact n)
    (n . zero? . branch (new const% 0) (new recur% n))))
(define-class const%
  (fields run))
(define-class recur%
  (fields n)
