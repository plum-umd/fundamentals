...
Unfortunately, code the BU students produced is so convoluted that it
doesn't bear looking at.  All you have to go on is that the \verb|Set|
data structure obeys the following interface:
...
;; A Set implements:
;; - insert : String -> Set
;; adds the given string to the set, if it isn't already there
;; - remove : String -> Set
;; removes the given string from the set
;; if the given string is not in the set, produce the original set
;; - size : -> Number
;; count the elements of the set
;; - has : String -> Boolean
;; is the given String in the set?
\end{verbatim}
\noindent
...
\begin{quote}
If we insert an element into a set, then the resulting set is the
...
\begin{quote}
If we insert element $a$ into a set, and then remove element $b$ from
the resulting set, the result has the same size as if we do the two
...
(define (pred1 s i)
  (or (and (s . has i) (= (s . insert i . size) (s . size)))
      (and (not (s . has i))
           (= (s . insert i . size) (add1 (s . size))))))
...
(check-expect (pred2 empty-set "yes" "no"))

;; counter-example
(check-expect (pred2 empty-set "yes" "yes"))
\end{verbatim}
\end{solution}
\end{problem}
