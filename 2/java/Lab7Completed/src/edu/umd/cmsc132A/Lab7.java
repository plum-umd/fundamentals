...
class ConsLoB implements ListOfBall {
  ...
  public ListOfBall next() {
    // return new EmptyLoB();
    IBall nextFirst = first.next();
    if (nextFirst.outside()) {
      return rest.next();
    } else {
...
class BallWorld extends World {
  ...
  public World onMouseClicked(Posn mouse) {
    // return this;
    int bound = 10;
    int dx = r.nextInt(bound) * 2 - bound;
    int dy = r.nextInt(bound) * 2 - bound;
    Ball ball = new Ball(mouse.x, mouse.y, dx, dy);
    return new BallWorld(new ConsLoB(ball, balls));
  }
