...
and introduce yourself. If your partner is *not* present, let one of your TAs
know.

The two of you will work as a team to solve problems. At any time, one of you
will be the *Head* and the other will be the *Hands*. The *Head* does the
thinking and the *Hands* does the typing. *Hands* type only what the *Head*
tells them to, but you're free to discuss any issues that pop up. We'll have you
switch off during the lab to make sure each of you get practice problem solving,
dealing with syntax, and getting finger exercises on the keyboard.
...
@link["https://submit.cs.umd.edu"]{UMD CS Submit Server} so we can keep an eye on
lab attendance.
...
@link["https://download.racket-lang.org"]{Download}, install, and run DrRacket to
get started.

Help your partner install it on their machine if you're done first. Then, pick
the first *Head* and *Hands* and get continue on only one machine.

Explore DrRacket's interface. First, find out how to set the current
@link["https://docs.racket-lang.org/drracket/choose-language.html"]{Language} to the
Beginning Student teaching language (BSL). We'll tell you which language to use
at the beginning of each lab and problem set.
...
@larger{@bold{Ex 1}}: Define a function @tt{what-temp} that consumes a number (in degrees
Fahrenheit) and produces one of three strings: "cold" for cold temperatures
(say, less than 45), "hot" for hot temperatures (at or greater than 75), and
"comfy" for anything in between.
...
Imperial system) may prefer to use Celsius for their temperatures. Define a
function @tt{fahrenheit->celsius} that converts a number from degrees Fahrenheit
to degrees Celsius. Search for the conversion online if you don't know it
off-hand.
...
@larger{@bold{Ex 4}}: Swap *Head* and *Hands*!
...
*Head*). Copy and paste it into DrRacket's /definitions window/ and give it a
name (like Skip!).

Draw a frame the color green around your picture of Skip. As always, the
...
@larger{@bold{Ex 6}}: Define a function @tt{in-a-mood} that places Skip on
backgrounds of different colors based on Skip's mood. You can use whatever you
want to describe the mood, like a number from 0 to 100 if Skip's a bit
